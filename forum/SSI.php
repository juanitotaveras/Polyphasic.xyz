...
function ssi_welcome($output_method = 'echo')
{
	...
	/*	else
			echo ' <strong>', $context['user']['name'], ' </strong>', allowedTo('pm_read') ? ' ' .  ' <a href="' . $scripturl . '?action=pm">'  . '<span class="badge">' . $context['user']['messages'] . '</span>' . '<span id="top_envelope" class="glyphicon glyphicon-envelope"></span></a>' . $txt['newmessages4'] . ' ' . $context['user']['unread_messages'] . ' ' . ($context['user']['unread_messages'] == '1' ? $txt['newmessages0'] : $txt['newmessages1']) : '', ' ]';
	}  // if there's only one unread message (unread messages == 1), print are; else, 
	// Don't echo... then do what?! */
		else 
			echo '<span id="welcome_span" class="headertxt"><strong>', $context['user']['name'], '   </strong>',  ' <a class="dropdown-toggle" data-toggle="dropdown" href="' . $scripturl . '?action=pm">' . '<span id="top_envelope" class="glyphicon glyphicon-envelope"></span>' . '<span id="message_counter" class="badge">' . $context['user']['unread_messages'] . '</span></a>
<ul class="dropdown-menu" role="menu">
  <li><a href="' . $addr .'/forum/index.php?action=unread">Unread posts</a></li>
  <li><a href="' . $addr . '/forum/index.php?action=unreadreplies">New replies</li>
</ul>
...
function ssi_logout($redirect_to = '', $output_method = 'echo')
{
	...
	$link = '<a id="logout_link" href="' . $scripturl . '?action=logout;' . $context['session_var'] . '=' . $context['session_id'] . '">' . $txt['logout'] . '</a>';
...
*/
function forum_login($redirect_to = '', $output_method = 'echo') {
	global $scripturl, $txt, $user_info, $context, $modSettings;
...
//	if ($output_method != 'echo' || $user_info['is_guest'])
//		return $user_info['is_guest']; 
//	echo '<script>alert("cake");</script>';

	echo '
		<li id="login_cell">
		  <!-- Tigger the login Modal with a button -->
		  <button tdefaultbutton class="btn btn-default btn-sm" id="loginbtn">Login</button>
                </li> <!-- end login_cell -->';
/*
		        <!-- end -->
			<table border="0" cellspacing="1" cellpadding="0" class="ssi_table">
